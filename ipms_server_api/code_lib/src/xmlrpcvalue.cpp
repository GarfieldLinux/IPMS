...
namespace XmlRpc {
...
#ifdef _WIN32
	//VC6ÎÞstrtollº¯Êý
	__int32_t ivalue = strtol(valueStart, &valueEnd, 10);
    if (valueEnd == valueStart)
		return false;
    _type = TypeInt;
    //_value.asInt = ivalue;
	_value.asInt = _atoi64(valueStart);
#else
	__int64_t ivalue = strtoll(valueStart, &valueEnd, 10);
    if (valueEnd == valueStart)
		return false;
    _type = TypeInt;
    _value.asInt = ivalue;
  ...
  {
    char buf[256];
    snprintf(buf, sizeof(buf)-1, "%llu", _value.asInt);
    buf[sizeof(buf)-1] = 0;
    std::string xml = VALUE_TAG;
    xml += I4_TAG;
    xml += buf;
