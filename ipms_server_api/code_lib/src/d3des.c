...
//DES KEY
char key[]={0,8,1,1,0,8,1,1};
//加密协议
void EncryptProtocol(char* sData,long nLen)
{
	pthread_mutex_lock(&m_desMutex);
	unsigned char   byKey[8]   =   {0};
	char   szTempKey[20];   
	int i = 0;
	strcpy(szTempKey,   key);   
	makekey(szTempKey,   byKey);   
	deskey(byKey,   EN0);
	for(i = 0; i < (nLen/8 * 8); i += 8)   
		des((unsigned char*)sData + i,(unsigned char*)sData + i);
	pthread_mutex_unlock(&m_desMutex);
	return;	
}

//解密协议
void DecryptProtocol(char* sData,long nLen)
{
	pthread_mutex_lock(&m_desMutex);
	unsigned char   byKey[8]   =   {0};
	char   szTempKey[20];   
