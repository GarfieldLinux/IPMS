...
string city="";
string isp="";
vector <string> ipmscanshu ;
 while(p)
 {
   printf("%s\n",p);
   string str = p  ;
   ipmscanshu.push_back(str);
   p=strtok(NULL,d);
 }
printf("解析访问路径  \n");
/*
...
*/
printf("url 参数个数 %d \n",ipmscanshu.size());
if(ipmscanshu.size()!=0){
if(ipmscanshu[0] == "get"){
    printf("开始处理get api 的参数 \n");
    string result = ipms.feedbackview(ipmscanshu[1],ipmscanshu[2],ipmscanshu[3],ipmscanshu[4]);
    printf("%s \n",result.c_str()) ;
       process->response_code = 404;
       process->status = STATUS_SEND_RESPONSE_HEADER;
       string str= "HTTP/1.1 200 OK\r\nServer: DN-IPMS/1.0\r\nContent-Type: text/html\r\nConnection: Close\r\n\r\n"+result ;//必须200 wget 对http协议头比较挑剔  
       printf("%d \n",str.length());
       strncpy(process->buf, str.c_str(), str.length());
       send_response_header(process);
       handle_error(processes, "not found");
       return;
}else if(ipmscanshu[0] == "update"){
//更新的操作是阻塞的 只要更新没有完成  所有的URL请求 都会等待
        printf("开始比较文件\n");
        ipms.filecompare();
        process->response_code = 404;
        process->status = STATUS_SEND_RESPONSE_HEADER;
        string str= "HTTP/1.1 200 OK\r\nServer: DN-IPMS/1.0\r\nContent-Type: text/html\r\nConnection: Close\r\n\r\n" ;//必须200 wget 对http协议头比较挑剔  
        printf("%d \n",str.length());
        strncpy(process->buf, str.c_str(), str.length());
        send_response_header(process);
        handle_error(processes, "not found");
